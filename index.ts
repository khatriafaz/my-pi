 ...
 * afaz-pi Extension
 *
 * Personal extension for pi with custom commands and behaviors.
 * Includes update_plan (todo/checklist) tool matching Codex CLI behavior.
 * Includes conversation_resources for session-scoped important links/notes.
 */
...
// ─── Types matching Codex CLI's plan_tool.rs ───
...
// ─── Custom UI component for /plan command ───
...
export default function (pi: ExtensionAPI) {
	...
	// Register /exit command to cleanly exit pi
	...
	// Reconstruct plan state on session events
	pi.on("session_start", async (_event, ctx) => reconstructState(ctx));
	pi.on("session_tree", async (_event, ctx) => reconstructState(ctx));

	// Update status line with plan progress when active
	...
	// ─── update_plan tool (matches Codex CLI exactly) ───
	...
	pi.registerTool({
		...
		// Custom result rendering — checklist like Codex CLI
	...
	// ─── /plan command to view current plan ───

	pi.registerCommand("plan", {
		description: "Show the current plan",
		handler: async (_args, ctx) => {
			if (!ctx.hasUI) {
				// Print mode: just text output
				if (!currentPlan || currentPlan.plan.length === 0) {
